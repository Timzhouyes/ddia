...
> z: 文档型数据库适用于什么场景？个人认为是适用于非结构化的（数据和数据之间没有复杂网状联系），比较原子而且内部属性较多的数据。
...
> z:nosql 对于有关联的数据的支持是糟透了的。相当于不断的遍历……
...
> z:反规范化，会导致各种地方的值需要被同步更新，那么每次对于一个以上的更新操作都得使用事务。而在关系型数据库之中，因为有 id 和外键的关联，众多个值和一个东西相关，那么可以只改一次。
>
> 况且谁知道当年写那些 shit 代码的程序员到底在多少个地方用了相同的值？
...
> Z:也就是说，对于应用内部的数据而言，并不存在一个可以固定的模式（本身就不存在或者极易变化），可以使用文档型数据库并且在程序的业务代码之中决定业务的逻辑和数据的读取方式。
...
> z:只有不改变应用大小的时候才能原地执行，不然就需要额外开辟内存空间，**一次性读取**所需的全部内容就成了奢望
...
> z:mysql 之中虽然支持 json 格式，但是实际上对查询和更新而言并不友好。如果查询 json，实际上是走全表。想要在 json 上面加上相应的虚拟索引，但是我都加虚拟索引了，为啥不单独整一列拉出来……
>
> 参考：https://blog.csdn.net/weixin_31715171/article/details/113170696
...
> z:作者举了两个例子，一个是数据库之中，一个是前端渲染的 css。这两个例子之中声明式比命令式更好，我认为是因为数据库的实现方是对应的 db，前端的实现方是浏览器，实际上程序的编写者：
>
> 1. 不知道对应的实现应该怎么操作
> 2. 对实现没有掌握权，比如对方可以进行一个依赖的删除使我们这边命令式的老代码失效，或者背后做一些优化来优化特定的命令。
>
> 在这些情况下，隐藏实现细节的声明式编程肯定更好。但是如果可以掌握细节，想要深度优化或者对特定的场景使用一些 trick，那么就需要进行命令式的编程。
